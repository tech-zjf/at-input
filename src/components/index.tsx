...
const AtInput: React.FC<AtInputProps> = ({
  ...
  const handleInputObservation = useCallback(() => {
    ...
    const textBeforeCursor =
      (selection.focusNode as Text).data?.slice(0, selection.focusOffset) || "";
    // 更新焦点节点状态
  ...
  const handleUserSelect = (user: UserOption) => {
    ...
    // 折叠选择范围
    range.collapse();
    // 更新选中的用户列表状态，添加新用户时进行去重
    setSelectedUsers((prevSelected) => {
      // 检查新用户是否已存在
      const isUserExists = prevSelected.some(
        (selected) => selected.id === user.id
      );
      // 如果用户已存在，则返回原数组
      if (isUserExists) return prevSelected;
      // 如果用户不存在，则添加到数组中
